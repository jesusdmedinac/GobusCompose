...
fun fusedLocationFlow(
    ...
    locationClient.requestLocationUpdates(locationRequest, locationCallback, Looper.getMainLooper()).await()
    awaitClose {
...
fun <E> SendChannel<E>.offerCatching(element: E): Boolean {
    return runCatching { offerCatching(element) }.getOrDefault(false)
}
